...
const FileFooter: React.FC<FileFooterProps> = ({ file, fileSrc, index, isImage }) => {
	...
	return (
		<div className="relative ">
			<h5 className="text-[12px] mt-1 font-normal break-words text-black dark:text-slate-100">{result}</h5>
			{componentState.showFileSize && (
				<span data-testid="file-size" className="text-[10px] text-black dark:text-slate-100">
					{fileSize}
				</span>
			)}
			...
			{componentState.allowEditing && isImage && (
				<button
					className="float-right absolute top-1 right-5 text-gray-500 dark:text-white"
